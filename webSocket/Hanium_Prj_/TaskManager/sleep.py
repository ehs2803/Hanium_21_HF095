...
class Sleep_Detector(object):
    ...
    def __init__(self):
        ...
        self.sc = TaskManager.consumers.ChatConsumer()
        self.sc.connect()
        #start_server = websockets.serve(self.get_frame, "localhost", 9998)
    ...
    def get_sleep(self):
        if self.sleepDetection():                           # 졸음감지를 하면
            ...
            print(views.ID, formatted_data, views.USERNAME)
...
class Blink_Detector(object):
    # 생성자
    def __init__(self):
        self.video = VideoStream(src=0).start()#urllib.urlopen('http://127.0.0.1:8000/video')#cv2.VideoCapture('http://localhost:8000')                # 웹캠 연결
        self.image = self.video.read()    # 프레임 읽어오기  success : 연결 성공 여부  image : 프레임 값
    ...
    # 프레임에 대한 딥러닝 모델 예측
    def get_frame(self):
        self.image = self.video.read()                            # 프레임 읽어오기
        self.image = cv2.resize(self.image, dsize=(650, 550), fx=0.5, fy=0.5)   # 프레임을 높이, 너비를 각각 절반으로 줄임.
