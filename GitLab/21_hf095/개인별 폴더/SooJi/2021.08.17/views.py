...
def Test(request):
    ...
    comp_todos = CompleteList.objects.all()     # 완료된 Todo 테이블 로드
    context = {
        'todos': todos,
        'comp_todos': comp_todos
    }  # 딕셔너리형태로 context에 넣는다
...
def createTodo(request):
    ...
    # POST 요청시
    if request.method == 'POST':               # DB의 Todo테이블에 쓰고,
    ...
    return redirect('test')  # 처리 후 test.html로 돌아가기
...
def selectTodo(request):
    ...
    if request.method == 'POST':
        ...
        # 입력받은 월과 저장되어 있는 투두리스트 날짜 비교
        sel_todos = CompleteList.objects.filter(end_date__startswith=sel_month)
...
def editTodo(request, pk):
    ...
    # POST 요청시
    if request.method == "POST":
        todo.contents = request.POST['todoContent']  # 내용 수정 반영
        todo.save()  # 수정된 내용 저장
        return redirect(f'/test/{pk}')  # 해당 게시글 페이지로 이동

    # GET 요청시 게시글 수정 페이지(todo_edit.html) 리턴
    return render(request, 'todo_edit.html', {'todo': todo, 'user': user})
